...
///
/// `extra` holds sources the signer has chosen to trust beyond the origin and the
/// page's own inline scripts — a tag manager, an analytics host. They cannot be
/// derived from the build: nothing in `dist` records that an inline bootstrap will
/// go on to inject a script from somewhere else. Every entry widens the policy, so
/// the flag that supplies them is deliberately explicit and per-source.
pub fn csp_script_src(inline_hashes: &[String], extra: &[String]) -> String {
